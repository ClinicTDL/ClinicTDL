...
const loadDashboardData = async () => {
  ...
  try {
    ...
    for (let d = new Date(last15Start); d <= now; d.setDate(d.getDate() + 1)) {
      ...
      displayLabels.push(d.toLocaleDateString('th-TH', { day: '2-digit', month: 'short' }))
    }
  ...
  <body>
    <div style="display:flex; justify-content:space-between; align-items:flex-end;">
      ...
      <div style="text-align:right; font-size:12px;">
        <div>เดือนปัจจุบัน: ${new Date().toLocaleDateString('th-TH', {month:'long', year:'numeric'})}</div>
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div class="grid-4">
        ...
        <div class="card">
          ...
          <div class="card-diff ${summary.value.dispensedThisMonth.change >= 0 ? 'text-green' : 'text-red'}">
  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
      ...
      <div class="bg-white dark:bg-slate-800 border border-clinic-border dark:border-slate-700 rounded-xl p-4 flex flex-col gap-2 shadow-sm">
        ...
        <div
          class="text-xs font-medium flex items-center gap-1"
          :class="summary.dispensedThisMonth.change >= 0 ? 'text-emerald-500' : 'text-red-500'"
        >
